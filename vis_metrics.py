...
def convert2np(pred, label, data_type):
...
def obtain_super_class(data_type=InputType.NUMPY):
    if data_type == InputType.NUMPY:
        MetricClass = object
...
def get_flow_metric(data_type=InputType.NUMPY):
    MetricSuperClass = obtain_super_class(data_type)
    class FlowMetric(MetricSuperClass):
        ...
        def __init__(self, output_names=None, label_names=None, mask=None,
            ...
            """
            if data_type == InputType.MXNET:
                super(FlowMetric, self).__init__('FlowMetric',
...
def get_pose_metric(data_type=InputType.NUMPY):
    MetricSuperClass = obtain_super_class(data_type)
    class PoseMetric(MetricSuperClass):
...
def get_multitag_metric(data_type=InputType.NUMPY):
    class MultiTagMetric(obtain_super_class(data_type)):
    ...
    return MultiTagMetric


def get_seg_metric(data_type=InputType.NUMPY):
    class SegMetric(obtain_super_class(data_type)):
